...
int main(int argc, char* argv[], char* envp[])
{
    ...
    // First need to look at section headers to find bss
    ...
    for(uint32_t i = 0; i < header->shnum; i++){
        ret = read(fd, section_header, sizeof(SectionHeader));

        if(section_header->sh_type == 0x8){
            printf("Found bss\n");


            printf("sh_name: %08lx\n", (uint64_t)section_header->sh_name);
            printf("sh_type: %08lx\n", (uint64_t)section_header->sh_type);
            printf("sh_flags: %08lx\n", (uint64_t)section_header->sh_flags);
            printf("sh_addr: %08lx\n", (uint64_t)section_header->sh_addr);
            printf("sh_offset: %08lx\n", (uint64_t)section_header->sh_offset);
            printf("sh_size: %08lx\n", (uint64_t)section_header->sh_size);
            printf("sh_link: %08lx\n", (uint64_t)section_header->sh_link);
            printf("sh_info: %08lx\n", (uint64_t)section_header->sh_info);
            printf("sh_addralign: %08lx\n", (uint64_t)section_header->sh_addralign);
            printf("sh_entsize: %08lx\n", (uint64_t)section_header->sh_entsize);
            printf("\nSEPARATOR\n");
    ...
    bss_start = (uint64_t*)malloc(num_bss * sizeof(uint64_t));
    bss_end = (uint64_t*)malloc(num_bss * sizeof(uint64_t));
    ...
    for(uint32_t i = 0; i < header->shnum; i++){
        ...
        if(section_header->sh_type == 0x8){
            ...
            bss_start[cur_bss] = section_header->sh_addr;
            bss_end[cur_bss] = section_header->sh_addr + section_header->sh_size;

            printf("bss_start[%2d]: %08lx\n", cur_bss, bss_start[cur_bss]);
            printf("bss_end[%2d]: %08lx\n", cur_bss, bss_end[cur_bss]);
    ...
    for(uint32_t i = 0; i < header->phnum; i++){
        ...
        // TODO: Need to now not load in the bss sections
